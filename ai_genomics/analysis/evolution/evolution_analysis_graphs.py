...
EVOLVED_CLUSTERS = load_s3_data(
    bucket_name, "outputs/analysis/tag_evolution/dbpedia_clusters_timeslice_embed.json"
)
# load reduced embeddings
REDUCED_ENT_EMBEDS = load_s3_data(
    bucket_name, "outputs/analysis/tag_evolution/dbpedia_tags_reduced_embed.json"
)

ALL_CLUSTERS = list(
    set(
        list(
            itertools.chain(*[list(i.keys()) for i in list(EVOLVED_CLUSTERS.values())])
        )
    )
)


def get_tfidf_top_features(list_of_ents: List[str], n_top: int = 3) -> str:
...
def get_cluster_sizes(
    ...
    for year, clust_info in evolved_clusters.items():
        ...
        if clust:
            clust_size.append(
                (cluster, cluster_name_mapper.get(cluster), year, len(clust))
            )
    return clust_size


def get_final_cluster_list(
    clust: str, evolved_clusters: Dict[str, List[str]] = EVOLVED_CLUSTERS
):
...
def make_unique_cluster_names(all_clusters: List[str] = ALL_CLUSTERS) -> Dict[str, str]:
    ...
    for cluster_name, cluster_name_count in cluster_name_mapper_counts:
        if cluster_name_count > 1:
            name_mapper_indxs = [
                list(cluster_name_mapper.keys())[i]
                for i, j in enumerate(list(cluster_name_mapper.values()))
                if j == cluster_name
            ]
            unique_names = [
                f"{cluster_name}-{str(i)}" for i in range(cluster_name_count)
            ]
            unique_cluster_name_mapper.update(
                (dict(zip(name_mapper_indxs, unique_names)))
            )
        unique_cluster_name_mapper[
            list(cluster_name_mapper.keys())[
                list(cluster_name_mapper.values()).index(cluster_name)
            ]
        ] = cluster_name
...
def make_timestamp_df(
    ...
    return (
        pd.concat({k: pd.Series(v) for k, v in evolved_clusters[year].items()})
        .reset_index()
        ...
        .assign(
            year_cluster_introduced=lambda df: df.cluster.apply(
...
def generate_entity_cluster_timestamp_plot(
    ...
    """
    timestamp_df = make_timestamp_df(
        year,
    ...
    return (
        alt.Chart(timestamp_df, width=500, height=500)
        .mark_circle(size=60)
        .encode(
            x="x",
            y="y",
...
if __name__ == "__main__":
    ...
    early_timestamp = generate_entity_cluster_timestamp_plot(
        year="2010", cluster_name_mapper=cluster_name_mapper
    )
    mid_timestamp = generate_entity_cluster_timestamp_plot(
        year="2015", cluster_name_mapper=cluster_name_mapper
    )
    final_timestamp = generate_entity_cluster_timestamp_plot(
        year="2021", cluster_name_mapper=cluster_name_mapper
    )

    across_timestamps = early_timestamp | mid_timestamp | final_timestamp
    colors = [
        "#%06x".upper() % random.randint(0, 0xFFFFFF) for _ in range(len(ALL_CLUSTERS))
    ]
    ...
    configured_across_timestamps = pu.configure_plots(
        across_timestamps, chart_title, chart_subtitle
    )
    ...
    logger.info(
        f"The average number of years a cluster persists is {statistics.mean(cluster_ages)}"
    )
    logger.info(
        f"The median number of years a cluster persists is {statistics.median(cluster_ages)}"
    )
    logger.info(
        f"The maximum number of years a cluster persists is {max(cluster_ages)}"
    )

    most_common_persists = Counter(cluster_ages).most_common()
    logger.info(
        [
            f"there were {persist[1]} clusters that lasted {persist[0]} years"
            for persist in most_common_persists
        ]
    )

    chart_title = ["# of Macro Entities Persistent Over Time"]
    chart_subtitle = [
        "Most Macro Entities survive 1-2 years but some have persisted for up to 12 years."
    ]
    most_common_persists_df = pd.DataFrame(
        most_common_persists, columns=["# of Years persisted", "# of Clusters"]
    )
    chart = (
        alt.Chart(most_common_persists_df)
        .mark_bar()
        .encode(x="# of Years persisted", y="# of Clusters")
        .configure_mark(opacity=0.5, color=pu.NESTA_COLOURS[1])
    )
    ...
    for cluster in ALL_CLUSTERS:
        ...
        if len(years_persistent) >= 10:
            cluster_sizes = get_cluster_sizes(
                cluster, cluster_name_mapper=cluster_name_mapper
            )
            all_cluster_sizes.extend(cluster_sizes)
    all_cluster_sizes_df = pd.DataFrame(
        all_cluster_sizes, columns=["Cluster", "Cluster Name", "Year", "Size"]
    )
    chart_title = ["Persistent Cluster Size over Time"]
    chart_subtitle = ["The most persistent clusters have largely grown over time."]

    chart = (
        alt.Chart(all_cluster_sizes_df)
        .mark_line()
        .encode(
            x="Year",
            y="Size",
