...
#load entities
patent_ents, crunchbase_ents, gtr_ents, oa_ents = get_ai_genomics_patents_entities(), get_crunchbase_entities(), get_gtr_entities(), get_openalex_entities()
...
if __name__ == "__main__":
    logger.info('loading AI genomics DBpedia entities and datasets across all data sources....')
    ...
    new_timeslice_y = propagate_labels(ents_per_date_clusts[2015], ents_per_date_clusts[2016])
    propagated_labels = dict()
    for _ in range(5):
        timeslice_x = list(ents_per_date_clusts.keys())[_ + 1]
        timeslice_y = list(ents_per_date_clusts.keys())[_ + 2]
        new_timeslice_y = propagate_labels(new_timeslice_y, ents_per_date_clusts[timeslice_y])
