"""This script clusters AI genomics entity embeddings at successive 
...
# load entities
patent_ents, crunchbase_ents, gtr_ents, oa_ents = get_ai_genomics_patents_entities(
), get_crunchbase_entities(), get_gtr_entities(), get_openalex_entities()
...
def filter_data(data: pd.DataFrame,
    ...
    data = (data.query(query).reset_index(drop=True).rename(
        columns={date_col: 'date', id_col: 'id'}))
...
def timestamp_entities(
    ...
    for min_date in date_range:
        ...
        for df, ents in zip(list_of_dfs, list_of_ents):
            ...
            ents_subset = {k: [i[0] for i in v]
                           for k, v in ents.items() if k in df_ids}
            all_ents.extend(
                list(set(list(itertools.chain(*ents_subset.values())))))
...
def propagate_labels(
    ...
    """
    perm_dists = [[(clust_x, clust_y, jaccard_similarity(ent_x, ent_y))
                   for clust_y, ent_y in timeslice_y.items()] for clust_x, ent_x in timeslice_x.items()]
    perm_dists = [[x for x in perm_dist if x[2] != 0]
                  for perm_dist in perm_dists]

    perm_dists_df = pd.DataFrame(itertools.chain(
        *perm_dists), columns=['timeslice_x_cluster', 'timeslice_y_cluster', 'jaccard_score'])
    new_y_clusters = dict()
    for timeslice, timeslice_info in perm_dists_df.groupby('timeslice_x_cluster'):
        max_score = max(timeslice_info['jaccard_score'])
        y_cluster = timeslice_info[timeslice_info.jaccard_score
                                   == max_score].timeslice_y_cluster.values[0]
...
if __name__ == "__main__":
    ...
    patents_filtered = filter_data(
        data=patents, query='~grant_date.isna()', date_col='grant_date', id_col='publication_number')
    ...
    #oa = load_s3_data(bucket_name, 'outputs/openalex/openalex_works.csv')
    #oa_filtered = filter_data(data=oa, query='ai_genomics == True', date_col='publication_date', id_col='work_id')
    # filter these DBpedia ents because the OA entity dict is very large
    #oa_ents = {k:v for k, v in oa_ents.items() if k in list(oa_filtered.id)}
    #logger.info('loaded and filtered openalex data')

    # timeslice entities
    ents_per_date = timestamp_entities(list_of_dfs=[patents_filtered, crunchbase_filtered, gtr_filtered, oa_filtered], list_of_ents=[
    ...
    best_ks = get_best_k(reduced_entities=ent_embeds_lookup,
                         ents_per_date=ents_per_date)
    ...
    save_to_s3(bucket_name, ent_embeds_lookup,
               'outputs/analysis/tag_evolution/dbpedia_reduced_embeds.json')
    save_to_s3(bucket_name, propagated_labels,
