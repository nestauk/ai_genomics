...
if __name__ == "__main__":
    ...
    patent_ents, crunchbase_ents, gtr_ents = clean_entities(get_ai_genomics_patents_entities()), clean_entities(get_crunchbase_entities()), clean_entities(get_gtr_entities())
    ...
    #prepare data
    all_ents = dict(pair for d in [patent_ents, crunchbase_ents, gtr_ents] for pair in d.items())
    all_dfs = pd.concat([patents_filtered[['id', 'date']], crunchbase_filtered[['id', 'date']], gtr_filtered[['id', 'date']]], axis=0).dropna()
    ...
    *[tuple(itertools.combinations(d, 2)) for d in list(all_ents.values())]
    ...
    ent_pair_years = {ent_pair:sorted(list(all_dfs[all_dfs.id.isin(ids)].date), reverse=False) for ent_pair, ids in ent_pairs.items()}
    ...
    #save output
    save_to_s3(bucket_name, subgraph_communities, 'outputs/analysis/tag_evolution/dbpedia_clusters_timeseries.pkl')