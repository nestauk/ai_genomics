...
# depending on what method might make sense, move this to a utils eventually
...
if __name__ == "__main__":
    ...
    patent_ents, crunchbase_ents, gtr_ents = clean_entities(get_ai_genomics_patents_entities(
    )), clean_entities(get_crunchbase_entities()), clean_entities(get_gtr_entities())
    ...
    all_dfs = pd.concat([patents_filtered[['id', 'date']], crunchbase_filtered[[
                        'id', 'date']], gtr_filtered[['id', 'date']]], axis=0).dropna()
    ...
    ent_pair_years = {ent_pair: sorted(list(all_dfs[all_dfs.id.isin(
        ids)].date), reverse=False) for ent_pair, ids in ent_pairs.items()}
    logger.info(
        'extracted date entity pair first appeared across all data sources')
    ...
    # save output
    save_to_s3(bucket_name, subgraph_communities,
