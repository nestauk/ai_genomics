...
from ai_genomics.getters.patents import get_ai_genomics_patents,get_ai_genomics_patents_entities
...
#load entities and patents data
patent_ents, crunchbase_ents, gtr_ents = get_ai_genomics_patents_entities(), get_crunchbase_entities(), get_gtr_entities()
...
def filter_data(data: pd.DataFrame,
    ...
    data = (data.query(query).reset_index(drop=True).rename(columns={date_col: 'date', id_col: 'id'}))
...
def generate_entities_embedding_lookup(
    entities: List[str]
) -> Dict[str, np.array]:
...
def get_best_k(reduced_entities:Dict[str, np.array], 
                ents_per_date:Dict[str, List[str]]
    ) -> List[int]:
    """Identifies the optimal number of clusters per timeslice based
    on the the gradient change for k between 10-70 at 5 intervals.
...
def update_labels(new_labels_dict:Dict[int, int], timeslice_dict: Dict[int, List[str]]) -> Dict[int, List[str]]:
...
def propagate_labels(
    ...
    min_jaccard_score:float=0.5) -> Dict[int, List[str]]:
    ...
    """
    perm_dists = [[(clust_x, clust_y, jaccard_similarity(ent_x, ent_y)) for clust_y, ent_y in timeslice_y.items()] for clust_x, ent_x in timeslice_x.items()]
    perm_dists = [[x for x in perm_dist if x[2] != 0] for perm_dist in perm_dists]

    
    perm_dists_df = pd.DataFrame(itertools.chain(*perm_dists), columns=['timeslice_x_cluster', 'timeslice_y_cluster', 'jaccard_score'])
    new_y_clusters = dict()
    for timeslice, timeslice_info in perm_dists_df.groupby('timeslice_x_cluster'):
        max_score = max(timeslice_info['jaccard_score'])
        y_cluster = timeslice_info[timeslice_info.jaccard_score == max_score].timeslice_y_cluster.values[0]
        ...
        if max_score > min_jaccard_score:
            ...
            logger.info(f'replace {y_cluster} with {timeslice} at t + 1 based on jaccard similarity.')
    
    #update timeslice_y labels
    return update_labels(new_y_clusters, timeslice_y)

if __name__ == "__main__":
    #ai in genomics entities
    patents_filtered = filter_data(data=patents, query='~grant_date.isna()', date_col='grant_date', id_col='publication_number')
    crunchbase = load_s3_data(bucket_name, 'outputs/crunchbase/crunchbase_ai_genom_comps.csv')
    crunchbase_filtered = filter_data(data=crunchbase, query='ai_genom == True', date_col='founded_on', id_col='id')
    ...
    #timeslice entities
    ents_per_date = timestamp_entities(list_of_dfs=[patents_filtered, crunchbase_filtered, gtr_filtered], list_of_ents=[patent_ents, crunchbase_ents, gtr_ents])
    ...
    best_ks = get_best_k(reduced_entities=ent_embeds_lookup, ents_per_date=ents_per_date)
    ...
    for year_indx in range(len(years) - 1):
        timeslice_x = ents_per_date_clusts[years[year_indx]]
        timeslice_y = ents_per_date_clusts[years[year_indx + 1]]
        new_timeslice_y = propagate_labels(timeslice_x=timeslice_x, timeslice_y=timeslice_y)
        propagated_labels[years[year_indx + 1]] = new_timeslice_y
    logger.info('propogated cluster labels across timestamps based on jaccard similarity.')
