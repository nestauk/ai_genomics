...
def get_openalex_sample_concepts(oa_sample: pd.DataFrame = OA_SAMPLE) -> pd.DataFrame:
    ...
    oa_tags = []
    for sample in list(oa_sample["api_url"]):
        oa_response = requests.get(sample)
        if oa_response.ok:
            if oa_response.headers["Content-Type"] == "application/json":
                ...
                if "concepts" in oa_data.keys():
                    ...
                    oa_tags.append([sample, tags, scores, levels])
                else:
                    logger.info(f"no concepts found for {sample}")
            else:
                logger.info("response is not in json")
        else:
            logger.info(f"response code not 200 - {oa_response}")

    oa_with_concepts = pd.DataFrame(
        oa_tags, columns=["api_url", "tags", "scores", "levels"]
    )
    oa_sample_with_concepts = pd.merge(oa_with_concepts, oa_sample, on="api_url")
    ...
    for sample_dir in sample_dirs:
        if tags_dir in sample_dir:
            if sample_dir.endswith(".csv"):
                ...
                sample_with_tags["data_source"] = sample_dir.split("/")[-1].split("_")[
                    2
                ]
