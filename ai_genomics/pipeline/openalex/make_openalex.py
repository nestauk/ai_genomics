# Script to generate openalex definitions and save outputs
...
def flag_ambiguous(abstract: str, work_id) -> bool:
...
def filter_works(works_meta: pd.DataFrame, abstracts: Dict) -> pd.DataFrame:
    ...
    return (
        ...
        .assign(
            ambiguous=lambda df: [
                flag_ambiguous(abstracts[work_id], work_id) for work_id in df["work_id"]
            ]
        )
...
if __name__ == "__main__":
    ...
    all_works = work_metadata(
        "artificial_intelligence", range(2012, 2023)
    ).drop_duplicates("work_id")
    all_concepts = work_concepts(
        "artificial_intelligence", "concepts", range(2012, 2023)
    )
    all_ai_mesh = work_concepts("artificial_intelligence", "mesh", range(2012, 2023))
    all_abstracts = work_abstracts("artificial_intelligence", range(2012, 2023))
    ...
    # Read institute info
    ai_authors, genet_authors = [
        work_authorship(disc, range(2012, 2023))
        for disc in ["artificial_intelligence", "genetics"]
    ]

    ai_ids_combined = (
        set(all_works_provisional["work_id"])
        .union(genetic_ai_abstract_ids)
        .union(genetic_ai_definition_ids)
    )
    ...
    for ai_dataset, genom_dataset, var_name, table_name in zip(
        ...
        ["work_id", "doc_id", "doc_id", "id"],
        ["openalex_works", "openalex_concepts", "openalex_mesh", "openalex_institutes"],
    ):
        print(table_name)
        compiled_table = compile_oalex_dataset(
            ai_dataset, genom_dataset, id_list, var_name
        )
        compiled_table.to_csv(f"{OALEX_OUT_PATH}/{table_name}.csv", index=False)

        save_to_s3("ai-genomics", compiled_table, f"outputs/openalex/{table_name}.csv")

    # Abstracts
    all_relevant_ids = ai_ids_combined.union(genom_ids_combined).union(ai_gen_total_ids)

    compiled_abstracts = {
        k: v
        for k, v in {**all_abstracts, **abstracts_genetics}.items()
        if k in all_relevant_ids
    }

    save_to_s3(
        "ai-genomics", compiled_abstracts, "outputs/openalex/openalex_abstracts.json"
    )
